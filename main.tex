...
% ── Screenshot placeholder (Th3 recapture) ──────────────────────────
% The submitted screenshots are being recaptured under controlled
% conditions (synthetic identities, no personal paths, no DevTools
% counters, fixed zoom/theme, one scenario per capture). Until the
% replacements land, each slot is marked here so its position and target
% asset are unambiguous. Grep for \screenshotplaceholder to find them.
...
\newcommand{\screenshotplaceholder}[2]{%
  \fbox{\parbox[c][0.26\textheight][c]{\dimexpr#1-2\fboxsep-2\fboxrule\relax}{%
    ...
    \normalfont\itshape [screenshot placeholder: recapture per Th3]}}}
...
far more slowly than $N$ (\SIrange{128.1}{147.1}{\micro\second}).
Windowed catch-up carries \SI{320}{B} of raw consistency-path hashes in place of \SI{222528}{B} at a
one-month offline window, a \num{695}-fold reduction against a synthetic baseline that re-fetches a
current-head-anchored proof on each of the $W$ occasions. In a two-client demonstration, the linked
clients surface substitution warnings in band, while a separate cross-vantage auditor detects
equivocation.

These results show that DUET's combination of automatic, in-band key-substitution detection and
single-proof offline consistency can be added to a CONIKS-style messaging directory --- measured to
$10^7$ labels --- without changing the asymptotic complexity of the proofs it already serves, using
additional lookups of the existing proof shape; the thesis sets out what remains between the measured
prototype and a production deployment at Signal's scale.
\end{abstract}
