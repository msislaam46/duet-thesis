...
The evaluation reports component measurements of the verification chain, not a head-to-head comparison with a
fielded system. On a directory of up to $10^6$ labels the (VRF-indexed) lookup proof has a median
size of \SI{817}{B}, comparable to a dense Merkle-log inclusion proof at the same directory scale, and median
full client verification stays below \SI{150}{\micro\second} through $10^7$ labels, growing far more
slowly than $N$. A single windowed consistency proof is smaller than the evaluation's per-epoch baseline
(re-fetching one proof per missed epoch) by a factor of \num{695} at a one-month window. In a two-client demonstration the
linked clients raise substitution warnings in band and a separate cross-vantage auditor detects
equivocation.

These results show that an in-band peer-warning channel and a single distant-head consistency proof
can be added to a CONIKS-style messaging directory without changing its proof asymptotics. The Signal
Desktop integration remains a prototype: its advisory hook and deterministic session-secret stand-ins
demonstrate proof verification and warning propagation, not end-to-end security with live X3DH-bound
labels. The thesis sets out what remains between this measured prototype and a production deployment.
\end{abstract}
