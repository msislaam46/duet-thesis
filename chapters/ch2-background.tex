...
identity binding rather than every public value used in one session. \Cref{sec:ext-signal} records the
...
an authentic initial binding. \Cref{sec:ext-ratchet} records the key-schedule detail.
...
log~\cite{dowling2021meta,dowling2023acka}. The supporting measurements are preserved in
...
separation, and proof walkthrough are preserved in \cref{sec:ext-merkle}.

Although each proof is compact, a monitoring policy that requests one proof for every missed epoch
makes a returning client's total work grow with its offline period. This is the efficiency problem
addressed by DUET's windowed proof.
...
separates local lookup verification from cross-vantage equivocation detection.
...
histories~\cite[pp.~1--2]{hu2021merkle2}. The structural differences and terminology are recorded in
\cref{sec:ext-vkd-blocks}.
...
defined in this thesis. The per-system mechanisms, limitations, and citations are preserved in
...
messaging scale but disables client history monitoring in the cited configuration, Signal follows the
...
are preserved in \cref{sec:ext-acka}.
...
Proof construction also trades bandwidth against structural privacy. Signal's fixed-length
