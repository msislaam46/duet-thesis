...
defines a Reasonable Monitoring Window through distinguished log entries and supplies owner and contact
monitoring algorithms~\cite{ietf-keytrans-protocol}. DUET uses the same combined-tree model. Its
windowed mechanism verifies one RFC~9162 consistency proof from the client's last trusted head to the
current head.
...
independent review of Meta's AKD implementation reports that all identified findings were fixed at
retest~\cite{nccgroup2023akd}. None of the cited deployments provides the peer-warning channel
...
WhatsApp (AKD)~\cite{whatsapp2023whitepaper} & identity key & public auditing & No & Not provided in the cited configuration\newline key-history monitoring disabled & signed root and public write-once (WORM) audit log \\
...
Second, catch-up must avoid one proof per missed epoch. The IETF draft has clients monitor
distinguished log entries~\cite{ietf-keytrans-protocol}, while CONIKS requires a series of checks after
missed epochs~\cite[pp.~7--8]{melara2015coniks}. DUET verifies one consistency proof between the
client's last trusted head and the current head.
