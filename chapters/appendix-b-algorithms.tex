...
\Fn{\normalfont\textsf{AuditEpoch}($\mathit{server}$)}{
  ...
  \ForEach{not-yet-audited epoch $e\le \mathit{sth_{new}}.\mathit{epoch}$ \emph{(including $e=0$)}}{
    $\textsf{AuditTransition}(\mathit{server},e)$ \tcp*{directory delta of EVERY epoch, not only the newest / log growth}
  }
...
\Fn{\normalfont\textsf{AuditTransition}($\mathit{server},e$)}{
  ...
  $(r_{e},\mathit{ops}_e,\pi_{\mathrm{incl}})\gets \mathit{server}.\textsf{GetEpochDelta}(e)$ \tcp*{$\mathit{ops}_e$: declared \textsf{Insert}$(\ell,\mathit{kind},v)$ and \textsf{Rotate}$(\ell,\mathit{kind},v_{\mathrm{old}},v)$ only --- the format cannot express a deletion}
  \Req $\pi_{\mathrm{incl}}$ binds $r_{e}$ to leaf $e$ of the log the validated STH signs\;
  \lForEach{\textsf{Insert}$(\ell,\mathit{kind},v)\in \mathit{ops}_e$}{\Req $\mathit{mirror}.\textsf{get}(\ell)=\bot$ \tcp*[f]{A1: an insert targets an ABSENT label --- else an overwrite disguised as an insert}}
  \lForEach{\textsf{Rotate}$(\ell,\mathit{kind},v_{\mathrm{old}},v)\in \mathit{ops}_e$}{\Req $\mathit{kind}=\textsc{mutable}\ \wedge\ \mathit{mirror}.\textsf{kind}(\ell)=\mathit{kind}\ \wedge\ \mathit{mirror}.\textsf{get}(\ell)=v_{\mathrm{old}}$ \tcp*[f]{reject immutable rotation / kind-misdeclaration / wrong prior value}}
  $r'\gets$ root after applying $\mathit{ops}_e$ to a scratch copy of $\mathit{mirror}$ \tcp*{leaves hash as $\Hleaf(\mathit{kind}\concat\ell\concat v)$}
  \Req $r'=r_{e}$;\ \ commit the scratch copy to $\mathit{mirror}$ \tcp*{exact transition; commit only on full success (no poisoning)}
