...
\Fn{\normalfont\textsf{AuditTransition}($\mathit{server},e$)}{
  ...
  $(r_{e-1},r_{e},\mathit{updates}_e,\pi_{\mathrm{batch}})\gets \mathit{server}.\textsf{GetEpochDelta}(e)$\;
  \Req $r_{e-1},r_{e}$ are log leaves $e{-}1,e$ (inclusion proofs against the latest verified head)\;
  \Req $\pi_{\mathrm{batch}}$ recomputes $r_{e-1}$ and proves every $\mathit{label}\in\mathit{updates}_e$ absent there \tcp*{no overwrite}
  $r'\gets$ root after inserting every $(\mathit{label},\mathit{value})\in\mathit{updates}_e$ into the tree of $r_{e-1}$\;
  \Req $r'=r_{e}$ \tcp*{exact transition: nothing deleted, modified, or overwritten}
}
