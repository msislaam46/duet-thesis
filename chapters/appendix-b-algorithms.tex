...
\Fn{\normalfont\textsf{UpdateByUserId}($\mathit{user\_id},pk$)}{
  ...
  record \textsf{Insert} (fresh label) or \textsf{Rotate} (occupied label) in the epoch's declared delta\;
...
\Fn{\normalfont\textsf{UpdateRaw}($\mathit{label},\mathit{value}$)}{
  ...
  $\mathit{live}.\textsf{Insert}(\mathit{label},\mathit{value})$ \tcp*{dual-index/CEA labels; PoC unauthenticated}
  record \textsf{Insert}$(\mathit{label},\mathit{value})$ in the epoch's declared delta\;
}
...
\Fn{\normalfont\textsf{AuditTransition}($\mathit{server},e$)}{
  ...
  \Req $\pi_{\mathrm{batch}}$ recomputes $r_{e-1}$; every \textsf{Insert} label is proven absent there, and every \textsf{Rotate} label is proven present with exactly the declared $v_{\mathrm{old}}$ \tcp*{no undeclared overwrite}
  $r'\gets$ root after applying every operation in $\mathit{ops}_e$ to the tree of $r_{e-1}$\;
  \Req $r'=r_{e}$ \tcp*{exact transition: nothing beyond the declared operations}
  \tcc{a \textsf{Rotate} is legitimate only on the global (VRF) family. Labels are opaque, so the
       auditor cannot distinguish a lawful identity-key rotation from an abusive overwrite of a
       write-once label; what the replay guarantees is that no mutation is \emph{silent}. A declared
       overwrite of a raw label is forced into the open, where its owner detects it: a replaced
       lookup slot fails the self-check, a replaced warning slot still alerts, and a replaced CEA
       fingerprint yields \textsc{Fork} (\cref{ssec:design-detection})}
}
