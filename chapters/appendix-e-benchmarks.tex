...
figures are single-thread per-operation costs. The SHA-NI row matters for portability rather than
being incidental hardware detail: the SHA-256 implementation dispatches to the hardware instruction
at run time, and on a host without it every hash-bound metric (the prefix and full verify, M5, the
M8 update, M9, and M13) is between roughly \num{1.3} and \num{4.6} times slower. This gap is measured
against a different processor and platform, so SHA-NI is the dominant \emph{plausible} factor rather
than a causally isolated one --- indeed the same auxiliary host is faster on the VRF-bound operations
(M10 prove \SI{106.5}{\micro\second} versus \SI{127.7}{\micro\second}), where SHA-256 is not on the
critical path. A run on such a host is not comparable with this one, and pinning the toolchain does
not make the timings portable. Timings are
...
\midrule
M2 search generation (dual-index)    & $10^3$ & 129.5 & 150.1 & 0.74 \\
M2 search generation (dual-index)    & $10^4$ & 129.3 & 145.3 & 0.74 \\
M3 prefix verify (dual-index)        & $10^3$ & 25.9  & 26.1  & 0.06 \\
M3 prefix verify (dual-index)        & $10^4$ & 25.9  & 26.1  & 0.38 \\
M3 VRF verify (dual-index)           & $10^3$ & 74.5  & 90.4  & 0.74 \\
M3 VRF verify (dual-index)           & $10^4$ & 74.5  & 89.9  & 0.61 \\
\textbf{M3 full client verify (windowed)} & $10^3$ & \textbf{128.4} & 153.9 & 0.76 \\
\textbf{M3 full client verify (windowed)} & $10^4$ & \textbf{128.3} & 144.1 & 0.72 \\
\bottomrule
\end{tabular}
\end{table}
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M5 detection check (dual-index) & $10^3$ & 1.6 & 2.1 & 0.15 \\
\bottomrule
\end{tabular}
\end{table}
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M8 lookup throughput (dual-index)    & $10^4$ & 130.2 & 148.4 & 0.98 \\
M8 update throughput (dual-index)    & $10^4$ & 150.8 & 175.1 & 1.02 \\
M9 key rotation (dual-index)         & $10^3$ & 149.2 & 176.0 & 0.84 \\
M9 key rotation (dual-index)         & $10^4$ & 150.2 & 166.9 & 0.92 \\
M9 epoch commit (windowed)           & $10^3$ & 468.4 & 562.7 & 6.40 \\
M9 epoch commit (windowed)           & $10^4$ & 6004.3 & 6697.0 & 51.13 \\
M10 VRF prove (isolated)             & ---    & 127.7 & 130.7 & 0.61 \\
M10 VRF verify (isolated)            & ---    & 81.4  & 83.9  & 0.44 \\
M11 handshake overhead (windowed)    & $10^3$ & 128.4 & 153.9 & 0.76 \\
M11 handshake overhead (windowed)    & $10^4$ & 128.3 & 144.1 & 0.72 \\
\bottomrule
\end{tabular}
\end{table}

M8 throughput follows as $10^6 / \text{median}_{\mu s}$: approximately $7{,}680$ lookups/s and
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M12 audit epoch (windowed)  & $10^4$ & 29.0 & 29.1 & 0.24 \\
M12 detect fork (windowed)  & $10^4$ & 27.4 & 27.5 & 0.13 \\
\bottomrule
\end{tabular}
\end{table}
...
Server full search (median / p95, \si{\micro\second}) & 113.6 / 206.3 & 136.0 / 2644.4$^{\dagger}$ \\
Prefix verify (median / p95, \si{\micro\second}) & 25.9 / 26.4 & 26.0 / 27.0 \\
VRF verify (median / p95, \si{\micro\second}) & 76.6 / 179.8 & 77.2 / 101.9 \\
Build time & 167.2\,s & 1{,}708.5\,s \\
RSS after build & 2{,}812\,MB & 21{,}520\,MB \\
\bottomrule
\end{tabular}
\end{table}

\noindent$^{\dagger}$The $10^7$ full-search 95th percentile (\SI{2644.4}{\micro\second}) sits an order
of magnitude above its \SI{136.0}{\micro\second} median, so a small fraction of lookups are far slower
...
The two 95\% confidence intervals ($\pm0.43$ and $\pm0.33\,\si{\micro\second}$) overlap, so no
practically meaningful difference between the modes is claimed at this resolution (the
\SI{0.4}{\micro\second} gap exceeds the smaller half-width, so an equivalence claim is not made
either): the chain memoization flattens the compressed-leaf cost that previously separated them.
...
M4  & \texttt{M4\_catchup\_cost}       & bandwidth: \texttt{rust\_benchmark\_results.csv} (\texttt{M4}); generation time (slope): Criterion \texttt{m4\_catchup\_cost} (see \texttt{results/PROVENANCE.md}) \\
...
M7  & \texttt{M7\_scale\_verification} & sizes / build / RSS: \texttt{rust\_benchmark\_m7.csv}; full-client-verify median: Criterion \texttt{m7\_scale\_verification/windowed\_full\_client\_verify} (see \texttt{results/PROVENANCE.md}) \\
