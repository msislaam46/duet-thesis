...
portability rather than being incidental hardware detail: the SHA-256 implementation dispatches to the
hardware instruction at run time, so a host lacking it takes a software path for every hash. An
auxiliary run of the same code on a different machine --- an Intel i9-9900K, which has neither SHA-NI
nor the same microarchitecture or clock --- measured the hash-bound operations (the prefix and full
verify, M5, the M8 update, M9, and M13) roughly \num{1.3} to \num{4.6} times slower. That difference
is attributable to the two changes together, the missing instruction and the different processor, and
this experiment cannot separate them: it varies both at once. It is reported as evidence that the
timings do not transfer between hosts, not as a measurement of SHA-NI's contribution. Consistently,
the same auxiliary host was \emph{faster} on the VRF-bound operations, where SHA-256 is off the
critical path. Pinning the toolchain does not make timings portable; they are
therefore reported as measurements of this configuration, not as properties of the scheme. The proof
sizes, being computed rather than timed, are hardware-independent.
...
The two 95\% confidence intervals ($\pm0.43$ and $\pm0.33\,\si{\micro\second}$) overlap, so the
ordering of the two medians is not interpreted; no equivalence claim is made either, since no
equivalence margin was defined or tested. The chain memoization flattens the compressed-leaf
