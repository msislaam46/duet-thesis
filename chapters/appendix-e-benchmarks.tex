...
Rust toolchain & \texttt{rustc 1.96.0-nightly} \\
Build commit & \texttt{46d867c9} \\
...
\midrule
M2 search generation (dual-index)    & $10^3$ & 107.2 & 124.2 & 0.48 \\
M2 search generation (dual-index)    & $10^4$ & 158.9 & 164.1 & 1.09 \\
M3 prefix verify (dual-index)        & $10^3$ & 26.0  & 26.1  & 0.33 \\
M3 prefix verify (dual-index)        & $10^4$ & 26.0  & 26.1  & 0.23 \\
M3 VRF verify (dual-index)           & $10^3$ & 73.8  & 82.7  & 0.59 \\
M3 VRF verify (dual-index)           & $10^4$ & 73.7  & 76.2  & 0.35 \\
\textbf{M3 full client verify (windowed)} & $10^3$ & \textbf{132.2} & 136.9 & 0.80 \\
\textbf{M3 full client verify (windowed)} & $10^4$ & \textbf{132.5} & 145.2 & 0.64 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{M5 --- MitM detection check latency.}
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M5 detection check (dual-index) & $10^3$ & 1.4 & 1.5 & 0.01 \\
\bottomrule
\end{tabular}
\end{table}
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M8 lookup throughput (dual-index)    & $10^4$ & 168.6 & 199.4 & 0.75 \\
M8 update throughput (dual-index)    & $10^4$ & 168.2 & 223.9 & 1.30 \\
M9 key rotation (dual-index)         & $10^3$ & 134.1 & 151.3 & 0.57 \\
M9 key rotation (dual-index)         & $10^4$ & 185.5 & 192.5 & 0.61 \\
M9 epoch commit (windowed)           & $10^3$ & 408.2 & 468.6 & 8.37 \\
M9 epoch commit (windowed)           & $10^4$ & 3449.2 & 4280.6 & 51.17 \\
M10 VRF prove (isolated)             & ---    & 106.3 & 109.5 & 0.81 \\
M10 VRF verify (isolated)            & ---    & 73.4  & 76.0  & 0.47 \\
M11 handshake overhead (windowed)    & $10^3$ & 132.2 & 136.9 & 0.80 \\
M11 handshake overhead (windowed)    & $10^4$ & 132.5 & 145.2 & 0.64 \\
\bottomrule
\end{tabular}
\end{table}

M8 throughput follows as $10^6 / \text{median}_{\mu s}$: approximately $5{,}930$ lookups/s and
$5{,}950$ updates/s, single-thread. M9 epoch commit scales roughly linearly in $N$ ($8.5\times$ for a
$10\times$ increase), reflecting the deep-clone snapshot discussed in \cref{ch:eval}. \emph{M11 is,
...
Operation (baseline) & $N$ & Median & p95 & CI $\pm$ \\
\midrule
M12 audit epoch (windowed)  & $10^4$ & 28.6 & 29.3 & 0.23 \\
M12 detect fork (windowed)  & $10^4$ & 27.6 & 29.1 & 0.50 \\
\bottomrule
\end{tabular}
\end{table}
...
Server full search (median / p95, \si{\micro\second}) & 149.2 / 199.4 & 154.7 / 214.4 \\
Prefix verify (median / p95, \si{\micro\second}) & 26.0 / 26.2 & 26.0 / 28.0 \\
VRF verify (median / p95, \si{\micro\second}) & 74.8 / 87.3 & 74.7 / 90.5 \\
Build time & 186\,s & 1{,}880\,s \\
Peak RSS & 2{,}041\,MB & 20{,}381\,MB \\
\bottomrule
\end{tabular}
\end{table}
