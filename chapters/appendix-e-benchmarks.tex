...
Per-check verification cost is $28.5\,\si{\micro\second}$ in session mode and
$28.3\,\si{\micro\second}$ in continuous mode (Criterion medians; both include server-side proof
...
M13 & \texttt{M13\_modes}              & \texttt{rust\_benchmark\_m13.csv} (write counts; the per-check verify timings are \texttt{M13\_modes} Criterion medians, not exported to CSV) \\
\bottomrule
\end{tabular}
\end{table}