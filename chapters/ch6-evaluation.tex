...
Key-substitution detection & \textbf{automatic in-band peer warning (established conversations)\newline comparison with the global VRF-bound identity entry when the directory is honest, supported by labels derived from different conversation secrets (first contact)} & manual Safety Number comparison and self-monitoring & optional AKV lookup and self-monitoring\newline no peer warning \\
