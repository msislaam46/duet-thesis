...
M12 measures two local verification operations. It does not measure a complete networked audit. The first
...
Resists offline identity recovery (VRF) & Yes (ECVRF, 80\,B) & Yes (DL-VRF, 96\,B) & Yes (ECVRF) \\
\bottomrule
\end{tabularx}
\end{table}
...
The published timings support only an order-of-magnitude comparison because the implementations and hardware differ. CONIKS reports a
...
\midrule
\hyperref[sc:threat-a1]{A1} server substitution & Owner self-check and dual-index warning (G1). & \cref{tab:tests-adversarial}. & Detected. \\
