...
Detection paths & \SI{27.8}{\micro\second} for a verified lookup that detects an owner-slot mismatch and \SI{31.1}{\micro\second} for the clean two-lookup path & Rust reconstruction excluding TypeScript, transport, polling, and registration \\
...
$N=10^{7}$, and the growth between them is the $\lceil \lg N \rceil$ term rather than any change in the
...
same directory and signed head rather than enlarging an individual proof.
...
\midrule
Lookup proof size & \textbf{817\,B median at $10^6$} (max 945, prefix and VRF, excluding STH) & 1{,}216\,B at $2^{32}$, 768--832\,B at $10^6$ derived (proof with and without signature) & 8{,}192\,B fixed sibling copath only (256$\times$32, no VRF) \\
Client lookup verification & \textbf{$\approx$\SI{128}{\micro\second} at $10^3$--$10^4$, \SI{139.2}{\micro\second} at $10^7$} (including VRF and STH signature) & a \SI{159}{\micro\second} path at $10^7$ plus about \SI{400}{\micro\second} for the signature, about \SI{559}{\micro\second} in total (derived), excluding VRF & not published \\
Catch-up at $W{=}720$ & \textbf{320\,B} (1 windowed proof) & $\Theta(W)$, $\sim$530\,kB derived & Monitoring endpoint\newline No published byte figure \\
...
\toprule
Attack & Control and requirement & Evidence & Result \\
\midrule
...
\hyperref[sc:threat-a4]{A4} split-view equivocation & Cross-vantage auditor (G4). & Equivocation test and the M12 signed-head fork comparison. & Detected at the next scripted comparison. \\
