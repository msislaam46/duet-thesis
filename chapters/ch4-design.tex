...
committed epoch}, never the live tree, because only a committed prefix root has been appended to the
log, so only a committed root can be tied, through one chain of proofs, back to a signed head.
...
\end{aligned}
\end{equation}
In \cref{eq:design-labels}, $\mathit{PRK}$ is the extracted pseudorandom key. The terms
$\mathit{id}_{\mathsf{self}}$ and $\mathit{id}_{\mathsf{peer}}$ are the deriving party's own and peer
identifiers, and $\min$
...
party's key occupies its own slot. The parties' keys occupy separate lookup slots alongside the
shared warning slot.
...
on their next check and raise a warning. To make warnings distinguishable from noise, the
...
raises a warning (since no third party should be able to reach the slot at all, unauthenticated
...
is a signed, append-only extension of the state already trusted. The warning-slot lookup is
...
$\CEA$ & per-epoch labels and fingerprints & continuous mode retains ACKA's per-epoch CEA label-and-fingerprint construction. Session mode adds conversation-derived dual-index labels and a shared warning slot \\
...
VRF, so no new cryptographic dependency is introduced. Its measured cost in this prototype is reported
in \cref{ch:eval}.
