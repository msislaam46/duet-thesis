...
\cref{ch:eval} includes every check. \Cref{sec:design-detail-sth} records the extended
currency and head-binding argument.
...
\cref{def:g1-consistency} covers the surrounding detection cases. Warning publication, changes across
epochs, offline catch-up, and equivocation require the separate mechanisms described elsewhere.
...
\Cref{tab:design-x3dh-coverage} records the detailed coverage.
...
transient substitution. The complete argument is preserved in
...
hash. This gives publicly verifiable outsider-enumeration resistance and matches Signal's deployed
...
cost is one log-inclusion path, quantified in \cref{ch:eval}. These choices provide one authenticated
chain from a directory entry to a signed history and retain direct non-inclusion proofs for the
warning slot. \Cref{sec:design-detail-alternatives} records the alternatives, and
