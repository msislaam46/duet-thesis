...
observable. A2 is censorship \emph{before} the warning commits: the server drops the
write so that no warning ever enters the log. Deleting a warning \emph{after} it has
committed is a separate tampering event rather than a form of A2: such a deletion is
inexpressible in the epoch's declared update set, so the live transition audit detects it when its
replay diverges. Separately, a specified and unit-tested persistence component lets the writer retain
evidence of the disappearance; this component is not wired into the live background monitor
(\cref{ch:design}). For an \emph{offline} client relying on
...
catches a post-commit warning deletion (tampering, distinct from A2's pre-commit
censorship); the warning's writer additionally holds deletion evidence, a specified and
unit-tested reference component that is not wired into the live monitor, so live detection of
post-commit deletion currently rests on the transition audit. Neither
...
addresses and the mechanism that delivers it. \cref{app:definitions} defines the formal statement that
makes each goal precise, and \cref{ch:eval} evaluates the implemented controls: G1 and G2 are
demonstrated by the adversarial and catch-up experiments, G3 rests on the VRF pseudorandomness
assumption with RFC~9381 conformance evidence, and G4 is demonstrated under the stated second-vantage
condition.
...
\midrule
G1 key-substitution detection & A1, A3, A5 & dual-index warning for A1/A5 (bidirectional); divergent labels + global VRF identity binding for A3; identity re-fetch + predecessor cross-signature for A5 & no \\
