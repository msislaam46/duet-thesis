...
required verified lookups and, where the peer's warning is needed, that warning write commits. For
established-pair attacks A1 and A5, either party must be able to detect the substitution.
...
The model excludes prevention, endpoint compromise, operator query privacy, group messaging, and
multi-device keychains. A refusal or dropped response is reported as an unavailable or incomplete
check rather than successful verification. These boundaries and possible extensions are examined in
\cref{ch:discussion}.
