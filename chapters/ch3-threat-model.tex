...
continuous channel but abstracts the log behind a security definition rather than realising a
...
abstract, security-defined log that ACKA leaves to be realised. The \emph{client--network} boundary is
...
(\cref{sec:bg-vkd}), and it lies outside ACKA's detection theorem, which assumes a Logsec-secure log rather than realising one against such a server.
...
capabilities. The purpose of key transparency is not to prevent such interference
--- no cryptographic directory can force a hostile network to deliver packets ---
but to convert silent compromise into detectable misbehaviour: a censored write, a
