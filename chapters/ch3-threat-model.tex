...
semi-trusted. It is relied upon for availability but assumed to act maliciously when it
...
(\cref{sec:bg-vkd}), and it lies outside ACKA's detection theorem, which assumes a $\Logsec$-secure log rather than constructing one against such a server. The same semi-trusted, potentially equivocating operator is assumed in CONIKS~\cite{melara2015coniks}, SEEMless~\cite{chase2019seemless}, Parakeet~\cite{malvai2023parakeet}, and the IETF Key Transparency architecture. Deployed messengers rely on such a service to return users' public keys, and documented fake-key attacks show that its compromise or coercion is a practical threat~\cite{yadav2022fakekey}.
...
A1 & malicious server & substitute a user's key & substitution detected by the affected parties \\
A2 & malicious server & substitute a key and censor the warning & censorship must not appear as successful verification \\
A3 & network (Dolev--Yao) & substitute a key in transit & in-transit substitution detected \\
A4 & malicious server & equivocate (split-view) & conflicting signed views exposed \\
A5 & malicious server & substitute during a key rotation & unauthorized rotation must not be accepted as authentic \\
X1 & endpoint holder & impersonate using the victim's key & outside the directory boundary \\
\bottomrule
\end{tabularx}
\end{table}
...
inside a rotation. The peer re-fetches the identity entry at the global VRF-derived label. If the
identity key has changed, the predecessor cross-signature, the new key signed by the trusted
predecessor, determines whether the rotation is authorized. In the demonstrated A5 control, a missing
...
Finally, the model follows a trust-on-first-use (TOFU) approach.\label{sec:threat-fair} Peer-side
detection acts only after a key has been pinned. One limitation remains, inherited from the
trust-on-first-use assumption that also underlies ACKA. Detection establishes that the key seen now
differs from a key trusted earlier. It cannot vouch for the first key, before any trusted baseline
exists. A substitution that predates first contact, poisoning the initial pin, is not caught, because
there is nothing to compare against. This limitation is carried through the analysis and revisited in
the discussion of limitations (\cref{ch:discussion}).
...
required verified lookups and, where the peer's warning is needed, that warning write commits. For the
established-pair attacks A1 and A5, G1 provides bidirectional detection through the in-band warning.
...
directory slot is hidden from outsiders.

\paragraph{G4: Fork Detection.} An equivocating server (A4) that signs two
divergent heads at one epoch is detected by cross-vantage auditing when a valid, independent
second-vantage head is available. G4 is the only requirement that depends on the auditor-honesty
assumption (\cref{sec:threat-assumptions}).

% ============================================================================
\FloatBarrier
\section{Out of Scope}\label{sec:threat-scope}

The model leaves five boundaries to \cref{ch:discussion}: prevention, endpoint compromise, operator
query privacy, group messaging, and multi-device keychains. Prevention is not attempted, since a
determined operator or network can always refuse service. The contribution is that interference
cannot masquerade as successful verification, because a refusal or drop is reported as an unavailable
or incomplete check without its cause being attributed. Endpoint compromise (X1) lies outside any
directory mechanism. Query privacy from the operator is a separate property from the directory privacy
of requirement~G3. The pairwise design does not extend warning propagation to group messaging or the
multi-device keychains ELEKTRA targets, both revisited in \cref{ch:discussion}.
