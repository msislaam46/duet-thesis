...
out-of-scope endpoint boundary (X1), is set out in \cref{fig:eval-attacktree}.
...
established-pair attacks A1 and A5, detection is bidirectional, so an offline victim remains covered.
...
query privacy, group messaging, and multi-device keychains. The load-bearing point is that
interference cannot masquerade as successful verification, since a refusal or drop is reported as an
unavailable or incomplete check without its cause being attributed. Each boundary and its future-work
direction is developed in \cref{ch:discussion}.
