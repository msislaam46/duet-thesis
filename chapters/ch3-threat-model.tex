...
relative to this model, so the chapter is intentionally precise rather than
discursive. We begin by defining the system model and its actors
...
posture inherited from CONIKS and the IETF architecture (\cref{sec:bg-logs}), in
...
The adversary model gives rise to the attack scenarios summarized in
\cref{tab:threat-attacks}, ranging from the honest baseline (A0) through the
...
of the substitution through the in-band warning channel.
...
inexpressible in the epoch's declared update set, so it diverges the auditor's
transition replay, and the warning's writer retains the proofs that turn the
...
identity key rather than being accepted silently, and the dual-index warning serves as
...
endpoints, not the endpoints themselves. \Cref{fig:threat-tree} organizes these
scenarios as an attack tree, showing which are detected and which fall outside the
guarantee.

\begin{figure}[t]
\centering
\input{figures/tikz/fig-threat-tree}
\caption[Attack taxonomy.]{Attack taxonomy. Each in-scope scenario (A1--A5) is annotated with the
mechanism that detects it; the endpoint-compromise case (X1) is shown as the
out-of-scope boundary.}
\label{fig:threat-tree}
\end{figure}
...
\paragraph{Realism of the Malicious-Server Assumption.} This is the standard
...
reflects deployment reality: in every deployed end-to-end encrypted messenger the
key-distribution server is the single point of trust, and its compromise, whether
by intrusion or by legal compulsion, is a documented concern; fake-key attacks in
deployed messengers show that this is not a hypothetical
surface~\cite{yadav2022fakekey}. Assuming the server may misbehave is therefore a
...
Finally, the model follows a trust-on-first-use (TOFU) approach, as discussed in
\cref{sec:threat-fair}. The peer-side detection mechanism introduced in
\cref{ch:design} can detect changes only after a peer's key has been accepted and
pinned. It therefore does not authenticate the initial key exchange itself: an
adversary that substitutes the key before the first authenticated key is observed
causes the client to pin the substituted key as its baseline. This residual
limitation is acknowledged explicitly rather than assumed away and is revisited in
\cref{ch:discussion}.
...
adversary (A3) substitutes a key, an honest party detects the substitution within
at most one epoch of its next check. G1 is realized bidirectionally: the victim
detects the wrong key under its own name, and a peer that has pinned the victim's
key detects it on its next key-fetch; either can raise the in-band warning, so an
offline victim remains covered.

\paragraph{G2: Consistency Across Offline Windows.} A client returning after an
offline period can confirm that the log it now sees is an append-only extension of
the one it last verified, at a cost bounded by the endpoint log sizes rather than
growing with the length of the absence. This property ensures that intermittent connectivity does not impose a
...
\begin{itemize}
  ...
  \item \textbf{Multi-device keychains.} Of the kind ELEKTRA targets
  (\cref{sec:bg-vkd}); the model is single-device per identity.
\end{itemize}
