...
vantage point and cross-check the tree heads the server publishes. Their role in
this model is limited to fork detection (scenario A4); no other guarantee depends
on them, a separation made precise in \cref{sec:threat-assumptions}.
...
therefore stated separately. At least one honest auditor is assumed, for fork
detection only. This is required solely for scenario A4: detecting an equivocating
server requires a second, independent vantage point against which to compare tree
heads. It is not required for the dual-index warning mechanism. Detection of
server-side substitution (A1) and network man-in-the-middle (A3) is performed by
the communicating clients and depends on no auditor. These two claims are
distinct: the warning mechanism does not inherit the auditor-honesty assumption.
...
the one it last verified, at a cost that does not grow with the length of the
absence. This property ensures that intermittent connectivity does not impose a
