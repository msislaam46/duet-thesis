...
Refuse to serve proofs & \emph{detected}, not prevented: a refusal is an observable availability signal; the KT cannot force the server to serve & no gossip transport in the PoC (the prototype fetches a second vantage directly) \\
...
Client escalates to server writes & write-once policy rejects overwrites of occupied labels, but does not authenticate the first write to an empty label & first-write authorization (empty-slot squatting) left to deployment (\cref{ch:discussion}) \\
\bottomrule
\end{tabular}
\end{table}
