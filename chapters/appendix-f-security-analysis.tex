...
\begin{table}[ht]
\centering
\caption{MITRE ATT\&CK Enterprise techniques mapped to their key-transparency mitigations. Five
techniques are mitigated; T1552 is the explicit endpoint-compromise boundary (X1).}
\label{tab:sec-mitre}
\footnotesize
...
The techniques span the Collection, Credential Access, Defense Evasion, and Impact tactics. Every
technique except T1552 is mitigated by a key-transparency control; T1552 is the deliberate
endpoint-compromise boundary, out of scope for any directory mechanism (\cref{ch:threat}). The table
lists only techniques with a faithful ATT\&CK counterpart. Adversary behaviours specific to a
malicious \emph{directory} --- observing registration patterns, operating a parallel encrypted
channel, abusing the server's own write authority, or selectively censoring writes --- have no
precise Enterprise-technique match and are the thesis's own A1/A2 threats, analysed through the STRIDE
enumeration and the attack scenarios of \cref{ch:threat} instead.
...
Censor warning writes & compound-signal detection (A2): write-once + transition audit + persistence & client out-of-band alert \\
...
realising a distrusted one. The other three are availability functions the proof-of-concept does not
implement --- write rate-limiting, a published-epoch staleness timeout, and a gossip transport for
proof-omission --- each a deployment-layer engineering task rather than a gap in the scheme
(\cref{ch:discussion}).