...
Client escalates to server writes & write-once rejects overwrites of occupied labels; a first write to a dual-index or warning slot needs the shared secret an outsider lacks, and a race on a known label fails as a write-once violation on the honest registration & explicit writer authentication on the raw first-write endpoint left to deployment (\cref{ch:discussion}) \\
\bottomrule
\end{tabular}
\end{table}
