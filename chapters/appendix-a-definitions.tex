...
is caught.}
Each $\mathsf{commit}$ freezes a \emph{declared update set} $\Delta_t$: the epoch's
$\textsf{Insert}(\ell,\mathit{value})$ operations on fresh labels and
$\textsf{Rotate}(\ell,\mathit{value}_{\mathrm{old}},\mathit{value})$ operations, each rotation
disclosing the exact value it replaces; the format cannot express a deletion. The \emph{transition
audit} replays $\Delta_t$ onto the directory of the previous committed root $\prefroot{t-1}$,
requiring every \textsf{Insert} label to be proven absent there and every \textsf{Rotate} label to
be proven present with exactly the declared prior value, and accepts iff the result equals the
committed $\prefroot{t}$: any undeclared mutation, in particular any deletion, makes the
replay diverge. Because labels are opaque, the audit guarantees that no mutation is silent rather
than that every declared rotation is lawful; a declared overwrite of a write-once label is forced
into the open, where the label's owner detects it. Specified in \cref{ssec:design-detection};
realized by \textsf{AuditTransition} (\cref{alg:auditor}).
\end{definition}
