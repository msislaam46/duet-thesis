...
uniqueness of the VRF (\cref{def:vrf-security}) rather than label secrecy. Warning censorship (both a
dropped pre-commit write proven absent and a post-commit deletion) is handled by the write-once
policy, the transition audit, and the persistence check, and split-view equivocation by the
cross-vantage auditor (\cref{ssec:design-detection}). A single quantified reduction combining these
