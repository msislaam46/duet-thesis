...
warning label symmetric, so that both parties address the same warning slot, while the lookup labels are
...
slot stays provably empty, i.e.\ if it substitutes without any warning being raised.}
...
returns \textsc{Safe}. The scheme achieves Goal~G1 (Key-Substitution Detection)
if $\advantage{dual\text{-}det}{\Adv}=\Pr[\Expt{dual\text{-}det}{\Adv}=1]$ is negligible in
$\secpar$. For the raw dual-index path the argument rests on the labels being unpredictable to the
adversary: it relies on HKDF (\cref{def:hkdf}) behaving as a PRF keyed by an unknown, sufficiently
high-entropy conversation secret, on the unforgeability of the warning-MAC, on the collision
resistance of $\Hash$ (\cref{def:hash}), on the soundness of the inclusion and prefix proofs, and on
the unforgeability of the STH signature (\cref{def:sig}). The uniqueness of the VRF
(\cref{def:vrf-security}) is what binds the separate global VRF-indexed path, not the secrecy of the
raw dual-index labels. The
