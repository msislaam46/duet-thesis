...
(goal G3 of \cref{ch:threat}); pseudorandomness underlies directory privacy.
\end{definition}
...
m\rceil$ hash values, independent of the number of intervening epochs $W$.
\end{lemma}
...
bounded by the depths of the two trees, $\lceil\lg m\rceil$ and $\lceil\lg n\rceil$. The bound depends
on the endpoint sizes alone and not on $W=$ (number of epochs between the two visits), which is the
property exploited in \cref{ssec:design-catchup}.
\end{proof}
...
continuous example authentication notion $\CEAsec$ that captures detection of an active adversary who
...
extracts $\mathit{PRK}=\HKDFex(\varnothing,\sharedS)$ and then returns the three 256-bit labels
\begin{align*}
