...
\begin{itemize}
  \item \texttt{crate/} --- the Rust \texttt{duet} crate: library modules
  ...
  \item \texttt{signal-desktop/ts/kt/} --- the embedded TypeScript client (mirrored
  byte-for-byte by the standalone \texttt{signal-desktop-kt/} package).
\end{itemize}
...
\texttt{demo/run.ps1} --- whose subcommands are \texttt{up}, \texttt{server}, \texttt{auditor},
\texttt{build}, \texttt{attack}, \texttt{fork}, \texttt{stop-fork}, \texttt{delete},
\texttt{simulate-delete}, \texttt{stop}, and \texttt{reset} --- for the
