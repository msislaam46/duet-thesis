...
corresponding prototype functions, and \cref{tab:apiref-events} maps the four
user-interface verdict states to their DOM event names. The live client attaches only at key upload and
...
\begin{table}[ht]
\centering
\caption{User-interface verdict states and their DOM events.}
\label{tab:apiref-events}
\small
\begin{tabular}{@{}lll@{}}
\toprule
State & Meaning & DOM event \\
\midrule
grey  & pending (initial, no result yet) & none (initial state, no event) \\
green & key verified & \texttt{kt:verified} \\
amber & directory server unreachable & \texttt{kt:unreachable} \\
red   & MitM warning or operational timeout & \texttt{kt:warning} \\
\midrule
\multicolumn{3}{@{}p{12cm}@{}}{\emph{demonstration only:} inject a one-shot CEA ratchet fork into the
selected conversation \hfill \texttt{kt:cea-inject}} \\
\bottomrule
\end{tabular}
\end{table}

The four verdict states drive the conversation-header badge and banner. Grey is the renderer's
initial state and carries no event: a conversation for which no result has yet arrived simply
remains grey. The three result states are delivered as DOM custom events on \texttt{window}. A
green \texttt{kt:verified} event is bound to a specific peer and conversation and is honoured only
when both match the conversation currently displayed, so a stale success cannot mask a later
change. A red \texttt{kt:warning} is matched permissively and is never suppressed by such a
mismatch. The \texttt{kt:cea-inject} event fires once against the selected conversation to stage the
continuous-mode demonstration and is not part of the production surface.
...
A1a. Its console reports \texttt{[KT][self] [ALERT] identity entry does not match the local key}. The
right capture shows continuous CEA mode detecting a conflicting per-epoch fingerprint after a
one-shot ratchet-key injection into the selected conversation. Both paths raise the red warning
banner.}
...
reports \texttt{verified: append-only history and the independent vantage agree} and the client
self-check provides detection. The same capture shows A1b, an immutable-slot rotation, producing
\texttt{[alert] transition audit failed: replay did not reproduce the committed state}. In the
right capture, A4 raises \texttt{[alert] fork: two validly signed roots exist at one tree size}.}
