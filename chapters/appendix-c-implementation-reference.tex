...
\texttt{npm run demo}. A containerized path
...
Client and auditor log lines carry a structured \texttt{[KT]} prefix with a component tag (and, on
the client, the affected peer or label), so the captures in \cref{ssec:impl-demo} are legible without
post-processing. Verbose proof dumps and the attacker's target details are gated behind explicit
verbose and demonstration flags and are off by default, keeping an ordinary run's output to one line
per state change.
...
badge and banner. An independent auditor polls the server. The attacker uses a privileged testing
interface. \Cref{tab:apiref-hooks,tab:apiref-events} list the hook functions and verdict-event names.}
...
then verifies the peer slot, its own owner slot, and the shared warning slot through three
label-bound, proof-verified lookups (the peer slot $\ell_A$ it retrieves Alice
...
second root at the current tree size. The cross-vantage auditor detects the two validly signed roots,
...
\par\smallskip
\small (b) A4 cross-vantage fork verdict
...
against its own replica and accepting only if the replayed root equals the committed root. Its
cross-vantage step keeps a fork distinct from an unverified consistency proof: two validly signed
roots at one tree size are reported as \texttt{[alert] fork: two validly signed roots exist at one
tree size}, whereas a required proof that is missing, malformed, or invalid yields \texttt{[alert]
consistency unverified: required proof is missing, malformed, or invalid} and retains the baseline
rather than declaring a fork. The auditor prints each verdict once and re-emits a line only when the
verdict or the signed head changes, so a screenshot of a stable run is not flooded. This
