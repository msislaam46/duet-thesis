...
Affero General Public License v3 (AGPL-3.0)~\cite{libsignal,signal-desktop}. Building on AGPL-3.0
software makes the resulting work a derivative subject to the same licence: its source must remain
available, and, distinctively for the AGPL, that obligation extends to users who interact with a
modified version over a network, not only to those who receive a binary. The prototype is a
...
deployment would be released under AGPL-3.0. The independent Rust \texttt{kt} crate depends only on
permissively licensed libraries (the \texttt{sha2} and \texttt{hkdf} crates under the MIT/Apache-2.0
dual licence, and \texttt{curve25519-dalek} and \texttt{ed25519-dalek} under BSD-3-Clause), which are compatible with that
...
code. The contribution here adds no new cryptographic capability beyond standardized primitives
(the RFC~9381 ECVRF, RFC~5869 HKDF, and Ed25519) and is released as open, published source, so it
falls within the same well-established exception rather than raising a new control concern.
...
users are present. No personal data was processed in the course of the project, all measurements
were run on synthetic identifiers and benchmark data, so no obligations under the UK GDPR were
engaged, while the design itself advances the privacy interests those regimes exist to protect. A
production deployment would inherit one governance duty the prototype does not: any operational
logging of warnings or audit events must itself be data-minimised and access-controlled, so that
...
(\cref{sec:impl-strategy}), it is a single-machine artefact rather than a distributed deployment,
...
A dual-use dimension is unavoidable: robust messaging security protects everyone, including those
who would misuse it. The settled position in the security community, which this work adopts, is
that defensive integrity infrastructure of this kind is overwhelmingly to the public good, and that
weakening it to disadvantage bad actors would disadvantage the far larger population of ordinary
and at-risk users. Finally, the work followed responsible-disclosure norms in the small: the one
...
Conduct~\cite{iet-conduct}, and it is worth setting out concretely how.
...
rigour} is reflected in the exhaustive and property-based testing and the reproducibility measures
