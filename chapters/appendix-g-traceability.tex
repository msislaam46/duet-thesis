\chapter{Traceability Matrix}\label{app:traceability}

This appendix traces the thesis's main claims from motivation to evidence. For traceability, this
matrix uses four evaluation questions. First, RQ1 asks whether each modelled key substitution is detected and,
for an established conversation affected by an A1 or A5 substitution, whether the detecting client can notify its peer in band. Second, RQ2
asks whether adding that detection avoids enlarging an individual lookup proof, and what the measured
proof sizes and local component timings indicate about the component cost of detection. Third, RQ3 asks
whether offline catch-up avoids one proof per missed epoch and how the resulting proof size depends on
the two endpoint log sizes.
Finally, RQ4 asks whether the client verifier can integrate into the Signal Desktop codebase.
\Cref{tab:trace-rq} links each question to the corresponding design claim, mechanism, tests,
benchmarks, and evidence. \Cref{tab:trace-contrib} organizes the same evidence by the four
contributions of \cref{sec:intro-contributions}.

\begin{landscape}
\begin{table}[htbp]
\centering
\caption[Research-question traceability.]{Research question $\to$ claim $\to$ mechanism $\to$ test $\to$ benchmark $\to$ evidence.}
\label{tab:trace-rq}
\setlength{\extrarowheight}{0pt}
\renewcommand{\arraystretch}{0.9}
\setstretch{1}
\footnotesize
\begin{tabular}{@{}L{1.6cm}L{4.3cm}L{3.7cm}L{5.4cm}L{1.7cm}L{5.4cm}@{}}
\toprule
RQ & Design claim & Mechanism & Test or scenario & Benchmark & Evidence \\
\midrule
RQ1 & each modelled substitution is detected.\newline After honest establishment, A1 produces an
in-band peer warning. A5 integration remains partial.\newline A3 is detected locally by the global
VRF-bound identity check when the directory is honest &
A1: dual-index warning scheme (\cref{sec:design-dual}).\newline A2: compound warning-censorship
signal and post-commit controls.\newline A3: global VRF-bound identity check, honest
directory.\newline A5: identity re-fetch and predecessor cross-signature. &
A1, A2, A3, and A5 scenarios (\cref{tab:tests-adversarial}) and dual-index derivation tests
(\cref{tab:tests-rust-prim}) &
M17 (clean workflows) & \cref{sec:eval-security}: the specified adversarial cases produce their expected outcomes.
M17 asserts a clean verdict without a warning for every listed clean workflow \\
RQ2 & detection does not enlarge an individual lookup proof. Across the tested large-directory
sizes, median lookup-verification latency changes from \SI{134.6}{\micro\second} at $N=10^6$ to
\SI{152.8}{\micro\second} at $N=10^7$ &
combined-tree verification chain (\cref{ssec:design-verify}) &
prefix-tree, VRF, and Merkle-log tests (\cref{tab:tests-rust-prim,tab:tests-rust-server}) &
M1--M3, M7, M11, M16 & \cref{sec:eval-micro,ssec:eval-scale}: median proof sizes are
\num{817}\,B at $N=10^6$ and \num{945}\,B at $N=10^7$. M16 measures
\SI{27.7}{\micro\second} and \SI{30.8}{\micro\second} for reconstructed mismatch and clean
paths in the \texttt{duet} crate. The Signal Desktop path is not timed
(\cref{ssec:eval-detection}) \\
RQ3 & offline catch-up uses one consistency proof whose size is logarithmic in the endpoint log
sizes &
windowed consistency proof (\cref{sec:design-windowed} and \hyperref[lem:windowed-size]{Lemma~\ref*{lem:windowed-size}}) &
Merkle-log exhaustive-consistency and windowed-consistency tests
(\cref{tab:tests-rust-prim,tab:tests-rust-server}) &
M4 & \cref{ssec:eval-catchup}: at $W=720$, the one-proof construction contains \num{320}\,B of
raw consistency-path hashes, compared with \num{222528}\,B across the synthetic $W$-proof
baseline \\
RQ4 & the verifier integrates into Signal Desktop as a prototype warning path that does not block
session establishment &
Signal Desktop verifier and lifecycle hooks (\cref{sec:impl-signal}) &
client unit and wire-parity suites (\cref{sec:tests-ts}) and the live demonstration
(\cref{ssec:impl-demo}) &
Not applicable & \cref{sec:impl-signal,ssec:impl-demo}: the verifier and warning interface run in
the Signal Desktop fork, and the two-client demonstration exercises the integrated path \\
\midrule
Supporting & split-view equivocation is detected by comparing separately obtained signed heads &
independent-vantage signed-head comparison (\cref{alg:auditor}) &
the A4 scenario and auditor fork tests, which supply a staged second head from the same server (\cref{sec:tests-rust}) &
M12 & \cref{ssec:eval-auditor}: M12 measures a local comparison after one head is stored. It does
not measure acquisition from an independent vantage \\
Supporting & continuous mode requires more directory writes than session mode under the assumed
session interval &
CEA continuous mode (\cref{ssec:design-modes}) &
M13 mode experiment (\cref{sec:bench-modes}) &
M13 & \cref{ssec:eval-modes}: the analytic ratio ranges from $24\times$ to $720\times$ across the
specified epoch counts and assumed session interval. These counts are not measured Signal
traffic \\
Supporting & a committed warning cannot be removed silently &
write-once policy, transition audit, and persistence evidence
(\cref{ssec:design-detection}) &
write-once and undeclared-removal tests (\cref{tab:tests-rust-server}) and transition audit and
deletion-evidence tests (\cref{tab:tests-rust-system}) &
None & \cref{sec:eval-security}: post-commit warning removal is detected by transition replay,
with persistence evidence providing an additional deletion record \\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}

\begin{table}[ht]
\centering
\caption{Contribution kind $\to$ output $\to$ primary evidence.}
\label{tab:trace-contrib}
\setlength{\extrarowheight}{2pt}
\footnotesize
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.1cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
Contribution & Output & Primary evidence \\
\midrule
Conceptual & dual-index warning scheme and ACKA-log instantiation (\cref{sec:design-dual,sec:design-acka}) & expected in-band verdicts in the listed dual-index warning cases. The implemented directory and auditor use ACKA's secure-log interface (\cref{sec:eval-security}) \\
Cryptographic & windowed consistency proof: RFC~9162 distant-head proof applied to offline monitoring, size bound (\hyperref[lem:windowed-size]{Lemma~\ref*{lem:windowed-size}}) and the dual-index same-head consistency lemma as supporting analysis (\hyperref[lem:dualindex-consistency]{Lemma~\ref*{lem:dualindex-consistency}}) & same-head label consistency shown from proof soundness and collision resistance. \num{320}\,B raw path bytes at $W{=}720$, one proof for $W$ missed epochs, size logarithmic in endpoint log sizes (\cref{ssec:eval-catchup}) \\
Systems & the \texttt{duet} crate containing the directory server, cryptographic core, auditor
process, and benchmark harness, together with the Signal Desktop client component
(\cref{ch:impl}) & fixed vectors and live integration tests establish agreement between the crate
and client component, while the two-client demonstration exercises the integrated warning path
(\cref{ssec:impl-demo}) \\
Empirical & benchmark evaluation M1--M17 and the STRIDE/MITRE evaluation
(\cref{ch:eval,app:security}) & detection reuses the existing lookup-proof format, while median latency
for the measured client-side lookup-verification chain remains below \SI{160}{\micro\second} across the tested sizes
(\cref{sec:eval-micro}). The adversarial suite produces the expected outcome for every specified
detection scenario. For A5, crate cross-signature tests and Signal identity-re-fetch/shared-warning
tests remain separate. They do not establish an integrated predecessor-authenticated warning path
(\cref{sec:eval-security}) \\
\bottomrule
\end{tabularx}
\end{table}

\section{Legal, Social, Ethical, and Professional Traceability}\label{sec:trace-lsepi}

\Cref{tab:lsepi-mapping} summarizes the responsibilities and project responses discussed in
\cref{ch:lsepi}.

\begin{table}[p]
\centering
\caption{Legal, social, ethical, and professional responsibilities and project responses.}
\label{tab:lsepi-mapping}
\footnotesize
\setlength{\extrarowheight}{2pt}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
Instrument or consideration & Relevance & Implication & Project response \\
\midrule
AGPL-3.0 (libsignal and Signal Desktop) &
the client component modifies Signal Desktop, which depends on libsignal; both upstream projects are AGPL-3.0~\cite{libsignal,signal-desktop} &
conveying modified source or object code must satisfy the applicable licence conditions~\cite[Sections~5--6]{agpl3}; if the modified version supports remote network interaction, the licence requires an offer of the ``Corresponding Source'' to those users~\cite[Section~13]{agpl3} &
the prototype was evaluated locally, upstream notices were retained, and no upstream contribution was submitted \\

UK GDPR (pseudonymised account data) &
the directory server receives raw service identifiers and stores account-linked pseudonymised data &
the data remains personal data for an operator holding the additional identifying information~\cite{ico-pseudonymisation} &
production requires a documented purpose and lawful basis, restricted access to warning records, and defined retention periods~\cite{ico-lawfulness,ico-data-principles} \\

UK export controls (cryptographic software) &
the project uses publicly documented cryptography and open-source implementations &
public availability does not by itself establish an export-control exemption~\cite{uk-cryptonote} &
no classification was attempted; a production release requires assessment against the applicable list and current guidance~\cite{uk-export-lists} \\

BCS Code of Conduct &
public interest, professional competence and integrity, duty to the relevant authority, and duty to the profession~\cite{bcs-code} &
privacy, security, user wellbeing, competence, and accurate reporting must be considered~\cite{bcs-code} &
established libraries, RFC test vectors, adversarial tests, supervisory review, and explicit limitations \\

IET Rules of Conduct &
ethical principles, current knowledge, work within competence, and qualified professional opinions~\cite[Rules~2, 5, 6, and 26]{iet-conduct} &
professional opinions must be objective and state their limitations or qualifications~\cite[Rule~26]{iet-conduct} &
RFC conformance tests, explicit assumptions, and qualified claims; any upstream vulnerability would use Signal's official reporting route~\cite{signal-security-report} \\

Warning design and usability &
DUET presents amber availability notices and red warnings to users &
frequent or poorly prioritised alerts may be ignored~\cite{ewaschuk2016monitoring,laubheimer2022alertfatigue}, while users may not react as intended to an incorrect authentication result~\cite[Section~4.1]{haring2024whistleblower} &
amber identifies availability failures; red identifies integrity or consistency failures and the fail-closed operational timeout; usability and accessibility testing remain future work \\

Research ethics (controlled accounts and synthetic inputs) &
the demonstration used two researcher-controlled Signal accounts~(\cref{ssec:impl-demo}), while benchmark and adversarial inputs were synthetic~(\cref{sec:lsepi-social}) &
the evaluation involved no human participants~(\cref{sec:lsepi-social}) and intercepted no third-party traffic~(\cref{ssec:impl-demo}) &
the unmeasured infrastructure, energy, and access costs of independent auditing are stated as a deployment consideration \\
\bottomrule
\end{tabularx}
\end{table}

