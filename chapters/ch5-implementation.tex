...
The standalone \texttt{duet} crate implements the Rust component. Rust was chosen because comparable
systems use it and because the crate can be measured independently of Signal and
libsignal~\cite{malvai2023parakeet,facebook-akd}. \Cref{fig:impl-overview} shows the resulting
...
$\lceil \lg N \rceil$ non-empty sibling hashes. Update cost is a separate
...
RFC vectors check the ECVRF, while fixed cross-language vectors require byte-for-byte agreement for
label derivation, warning payloads, and CEA evolution. A live wire-parity suite checks the TypeScript verifier against signed heads, VRF outputs,
