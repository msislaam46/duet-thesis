...
Reproducibility relies on fixed cryptographic and sampling inputs and a recorded environment. Seeds
determine the VRF key, label placement, sampled users, and fresh state for each benchmark, while the
demonstration uses the administrative reset. The environment record identifies the hardware and the
versions of the operating system, compiler, and toolchain. \Cref{sec:apiref-repo} gives the \texttt{cargo test} and
\texttt{cargo bench} commands, repository layout, and runner scripts.
