...
\item[CEA / CEAsec] Continuous entity authentication and its security notion within
  ACKA; the continuous operating mode realises detection in this style.
...
\item[Dual-index warning scheme] This work's construction: two lookup labels and one shared warning
  label per conversation, derived from the shared secret, with the warning label acting as a tripwire
...
\item[MLS] Messaging Layer Security, the standardised group-messaging protocol; group support is
  future work.
...
\item[OPRF] Oblivious pseudorandom function (RFC~9497); proposed as future work for query privacy
  from the operator.
\item[Parakeet] A witness-quorum key-transparency system optimised for large-scale deployment.
