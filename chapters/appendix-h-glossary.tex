...
\item[Auditor] An independent party that observes the log from a second vantage point and cross-checks
  signed tree heads; required only for fork detection (\cref{app:algorithms}, Algorithm~7).
\item[CEA / CEAsec] The continuous example authentication primitive and its security notion within
...
\item[SEEMless] A key-transparency system with an ordered zero-knowledge directory; ships multi-path
  lookup proofs.
...
\item[Windowed consistency proof] A single consistency proof spanning an entire offline window,
  independent of its length (\cref{def:windowed}, \cref{lem:windowed-size}).
