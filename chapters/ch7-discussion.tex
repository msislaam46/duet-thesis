...
matched CONIKS proof, detector branches reuse raw-label proofs, and distant-head consistency replaces
a proof per missed epoch with one proof. These are component results rather than a
...
The guarantee begins after an honest baseline and therefore retains trust on first use. It covers
...
The first implementation priority is to
obtain the live X3DH or PQXDH conversation secret and the live Double Ratchet chain key from
libsignal, and to authenticate the party authorized to make the first raw-label write. A formal
analysis should then define a combined detection game covering the KDF, VRF, signed heads, warning
authentication, and append-only verification, alongside symbolic analysis and separate verification
of the client proof checker. Later replacement and deletion would remain governed by transition
auditing rather than by first-write authorization alone. The persistence check that detects later
removal is available as a tested reference component (\cref{sec:sec-suppression}). Its module status
is recorded in \cref{tab:impl-modules}.
...
Concurrency, serialization, reclamation, and memory use would
require measurement after structural sharing.

\subsection{Evaluation, Operations, and Usability}

A usability study should test interpretation of
the red warning and the amber notice, accessibility, escalation, and alarm fatigue,
continuing the ceremony research that motivated the design~\cite{vaziripour2017}. Production warnings
...
with its own identity, availability, and regulatory requirements.
